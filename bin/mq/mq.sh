if type -P mysql &>/dev/null ; then # This script only works if you have 'mysql' installed
  ...
  if [ "$OS" = "darwin" ] ; then # OS X only
    mqruntohtml (){
      rm /tmp/query.html
      mysql -H -u ${MYSQL_DEFAULT_USER} -D ${MYSQL_DEFAULT_DB} -t -vvv  -e "$@" | erb ~/Support/bin/mq/table_template.html.erb >> /tmp/query.html
      open /tmp/query.html
    }
    mqalltohtml (){
      rm /tmp/query.html
      mysql -H -u ${MYSQL_DEFAULT_USER} -D ${MYSQL_DEFAULT_DB} -t -vvv -e "select * from $@" | erb ~/Support/bin/mq/table_template.html.erb >> /tmp/query.html
      open /tmp/query.html
    }
  fi
  ...
  # Completion
  complete -o nospace -C ~/Support/bin/mq/complete_database_list.rb mqusedatabase mqdropdatabase
  complete -o default -C ~/Support/bin/mq/complete_table_list.rb mqall mqallcount mqallgrep mqfirst mqfields mqalltohtml mqallwhere
